...
void CNKeccak(ulong* md, uchar* in, uint len)
{
...
__kernel void cn0(__global uchar *input, __global uint4 *Scratchpad, __global ulong *states, ulong Threads, ulong Blob_len)
{
    ulong State[25];
...
#       endif
		...
		ulong nonce = (ulong)get_global_id(0);
		uchar *nonp = (uchar*)&nonce;

		for (int i = 0; i < 4; ++i)
		{
			input2[Blob_len - 8 + i] = nonp[i];
			
		}//take care of pointer alignment
		//printf("nonce used = %02x%02x%02x%02x%02x%02x%02x%02x",input2[Blob_len - 8],input2[Blob_len - 7],input2[Blob_len - 6],input2[Blob_len - 5],input2[Blob_len - 4],input2[Blob_len - 3],input2[Blob_len - 2],input2[Blob_len - 1]);

		CNKeccak( State, input2, Blob_len);
...
#   endif
    {
        ...
        for (int i = 0; i < ITERATIONS; ++i) {
			...
			uchar tmp2 = ((uchar*)(&a[0]))[1]; 
			uint table2 = 0x75310; 
			uchar index2 = (((tmp2 >> 3) & 6) | (tmp2 & 1)) << 1; 
			((uchar*)(&a[0]))[1] = tmp2 ^ ((table2 >> index2) & 0x33); 
...
__kernel void Skein(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
    ...
    if(idx < Threads)
    {
		...
		ulong tramp2;
		for(int i = 0; i < 8 ; i++)
			((uchar*)(&tramp2))[i] = ((uchar*)(&tramp))[7-i];

        if(tramp2 < Target)  //working
        {
...
__kernel void Blake(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
    ...
    if(idx < Threads)
    {
        ...
        // Note that comparison is equivalent to subtraction - we can't just compare 8 32-bit values
        // and expect an accurate result for target > 32-bit without implementing carries
        uint2 t = (uint2)(h[0],h[1]);
		ulong tramp = as_ulong(t);
		ulong tramp2;
		for(int i = 0; i < 8 ; i++)
			((uchar*)(&tramp2))[i] = ((uchar*)(&tramp))[7-i];

        if( tramp2 < Target) //works
        {
            ulong outIdx = atomic_inc(output + 0xFF);
            if(outIdx < 0xFF)
                output[outIdx] = BranchBuf[idx] + get_global_offset(0);
...
__kernel void Groestl(__global ulong *states, __global uint *BranchBuf, __global uint *output, ulong Target, ulong Threads)
{
    ...
    if(idx < Threads)
    {
        ...
        ulong tramp3;
		for(int i = 0; i < 8 ; i++)
			((uchar*)(&tramp3))[i] = ((uchar*)(&State[4]))[7-i];
