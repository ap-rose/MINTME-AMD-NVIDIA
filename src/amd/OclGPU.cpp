...
size_t InitOpenCLGpu(int index, cl_context opencl_ctx, GpuContext* ctx, const char* source_code, xmrig::Config *config)
{
    ...
    ctx->InputBuffer = OclLib::createBuffer(opencl_ctx, CL_MEM_READ_ONLY, 486, nullptr, &ret);
    if (ret != CL_SUCCESS) {
...
size_t XMRSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, uint32_t variant, uint64_t stnonce)
{
    cl_int ret;
	...
	for (int i = 0; i < 4; i++)
		input[input_len-4+i] = ((uint8_t*)(&stnonce))[i+4];
		
    size_t numThreads = ctx->rawIntensity;
	size_t Blob_len = input_len;

    if ((ret = OclLib::enqueueWriteBuffer(ctx->CommandQueues, ctx->InputBuffer, CL_TRUE, 0, 486, input, 0, nullptr, nullptr)) != CL_SUCCESS) {
	...
	// length for webchain
	if ((ret = OclLib::setKernelArg(ctx->Kernels[0], 4, sizeof(cl_ulong), &Blob_len)) != CL_SUCCESS) {
...
size_t XMRRunJob(GpuContext* ctx, cl_uint* HashOutput, uint32_t variant, uint64_t stnonce)
{
    cl_int ret;
    cl_uint zero = 0;
